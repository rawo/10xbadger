...
describe("BadgeApplicationService.acceptBadgeApplication", () => {
  it("accepts a submitted application successfully", async () => {
    ...
    const result = await service.acceptBadgeApplication(id, "reviewer-1", "Looks good");

    expect(result).toBeDefined();
    expect(result.id).toBe(id);
    expect(result.status).toBe("accepted");
    expect(result.reviewed_by).toBe("reviewer-1");
    expect(result.review_reason).toBe("Looks good");
  });
  ...
  it("throws NOT_FOUND when application does not exist", async () => {
    ...
    await expect(service.acceptBadgeApplication(id, "reviewer-1")).rejects.toThrow("NOT_FOUND");
  });
  ...
  it("throws INVALID_STATUS_TRANSITION when application not in submitted state", async () => {
    ...
    await expect(service.acceptBadgeApplication(id, "reviewer-1")).rejects.toThrow("INVALID_STATUS_TRANSITION");
  });
});
