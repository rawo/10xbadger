...
export const POST: APIRoute = async (context) => {
  try {
    ...
    if (!isAdmin) {
      const error: ApiError = { error: "forbidden", message: "Only admins may accept badge applications" };
    ...
    const validation = acceptBodySchema.safeParse(body);
    ...
    try {
      const updated = await service.acceptBadgeApplication(id, reviewerId, decisionNote);
      ...
      if (err instanceof Error) {
        ...
        if (err.message === "INVALID_STATUS_TRANSITION") {
          const apiError: ApiError = {
            error: "conflict",
            message: "Badge application cannot be accepted in its current state",
          };
      ...
      console.error("Error in POST /api/badge-applications/:id/accept:", err);
      try {
        await logError(context.locals.supabase, {
          route: "/api/badge-applications/:id/accept",
      ...
      const apiError: ApiError = {
        ...
        message: "An unexpected error occurred while accepting the badge application",
      };
      return new Response(JSON.stringify(apiError), { status: 500, headers: { "Content-Type": "application/json" } });
    }
  } catch (error) {
    // eslint-disable-next-line no-console
    console.error("Error in POST /api/badge-applications/:id/accept:", error);
    const apiError: ApiError = {
      error: "internal_error",
      message: "An unexpected error occurred while accepting the badge application",
    };
